...
export default function ProjectViewerPage() {
  ...
  return (
    <ErrorBoundary>
      <div className="min-h-screen bg-slate-950 text-slate-100 flex flex-col relative pb-14 sm:pb-8">
        <header className="border-b border-slate-800 bg-slate-900/80 backdrop-blur-xl sticky top-0 z-30">
          <div className="max-w-7xl mx-auto px-2.5 sm:px-6 lg:px-8 h-12 sm:h-16 flex items-center justify-between gap-1.5 sm:gap-4">
            <div className="flex items-center gap-1.5 sm:gap-4 min-w-0">
              <Link href="/dashboard">
                <Button
                  variant="ghost"
                  size="sm"
                  className="p-1 sm:p-2 text-slate-400 hover:text-white shrink-0"
              ...
              <div className="min-w-0">
                <div className="flex items-center gap-1.5 sm:gap-2">
                  <h1 className="text-sm sm:text-lg md:text-xl font-bold text-white tracking-tight truncate max-w-[110px] xs:max-w-[150px] sm:max-w-xs md:max-w-md">
                    {project.name}
                  </h1>
                  <button
                    onClick={handleCopyCode}
                    className="flex items-center gap-1 font-mono text-[10px] sm:text-xs px-1.5 sm:px-2 py-0.5 rounded-md sm:rounded-lg bg-sky-950/60 border border-sky-800/60 text-sky-400 hover:bg-sky-900/60 transition-colors shrink-0"
            ...
            <div className="flex items-center gap-1.5 sm:gap-3 shrink-0">
              <Button
                ...
                className="hidden sm:inline-flex text-xs py-1 px-2.5 sm:py-1.5 sm:px-3"
              >
                Código QR
              </Button>
        ...
        <main className="flex-1 max-w-7xl w-full mx-auto px-2 sm:px-6 lg:px-8 py-2 sm:py-4 lg:py-6">
          <PhotoGrid
            project={project}
